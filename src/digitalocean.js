...
module.exports.createDroplet = async (stream) => {
  const userData =
    "#!/bin/bash\n" +
    `wget ${config.files.username} --password ${config.files.password} https://${config.files.hostname}.angelthump.com:${config.files.port}/transcoder-image.sh -O /root/startup.sh\n` +
